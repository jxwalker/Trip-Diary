...
- Prefer real data already attached to the itinerary (e.g., from 
...
class RecommendationsService:
    async def get_recommendations(self, itinerary: Dict) -> Dict:
        """
        Generate recommendations based on itinerary using available REAL 
        data only.
        """
        # Prefer recommendations already added to the itinerary by real 
    ...
    async def update_with_preferences(self, recommendations: Dict, 
        ...
        if preferences.get("budget"):
            ...
            if budget == "low":
                recommendations["restaurants"] = [
                    r for r in recommendations["restaurants"] 
                    if r["price_range"] in ["$", "$$"]
                ]
            elif budget == "high":
                recommendations["restaurants"] = [
        ...
        if preferences.get("interests"):
            ...
            if "art" in interests:
                ...
                recommendations["attractions"] = sorted(
                    ...
                    key=lambda x: ("museum" in x["type"].lower() or 
                                   "art" in x["name"].lower()),
                    reverse=True
                )
