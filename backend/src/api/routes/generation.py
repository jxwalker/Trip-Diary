...
@router.get("/generation-stream/{trip_id}")
async def stream_generation(
    ...
    """
    Stream the itinerary generation process with Server-Sent Events
    ...
    Returns:
        StreamingResponse with generation updates
    """
    logger.info(f"Starting generation stream for trip {trip_id}")
    
    async def generate():
        try:
            # Send initial event
            yield f"data: {json.dumps({'type': 'start', 'message': 'Starting itinerary generation...'})}\n\n"
            ...
            # Reset and update status for new generation
            await database_service.update_processing_state(
                trip_id,
                status=ProcessingStatus.PROCESSING,
                progress=10,
                message="Analyzing trip details..."
            )
            yield f"data: {json.dumps({'type': 'progress', 'progress': 10, 'message': 'Analyzing trip details...'})}\n\n"
            
            # Check if we have itinerary data
            if not trip_data.itinerary:
                yield f"data: {json.dumps({'type': 'error', 'message': 'No itinerary data found'})}\n\n"
                return
            
            # Generate the guide
            yield f"data: {json.dumps({'type': 'progress', 'progress': 30, 'message': 'Finding attractions and restaurants...'})}\n\n"
            ...
            # Use the optimized guide service to generate
            try:
                # Generate guide with progress updates
                ...
                yield f"data: {json.dumps({'type': 'progress', 'progress': 80, 'message': 'Finalizing itinerary...'})}\n\n"
                ...
                yield f"data: {json.dumps({'type': 'complete', 'progress': 100, 'message': 'Itinerary generation complete!'})}\n\n"
            ...
            logger.error(f"Stream generation failed for trip {trip_id}: {e}")
